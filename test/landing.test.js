...
test("scaffoldLanding creates valid metadata and escapes JSX text", (t) => {
  var cwd = mkdtempSync(join(tmpdir(), "shipcli-landing-"));
  ...
  var generatedPackage = JSON.parse(readFileSync(join(cwd, "web/package.json"), "utf-8"));
  assert.equal(generatedPackage.name, "demo-cli-web");

  var page = readFileSync(join(cwd, "web/app/page.tsx"), "utf-8");
  assert.match(page, /Analyze &lt;code&gt; &amp; "ship"/);

  var layout = readFileSync(join(cwd, "web/app/layout.tsx"), "utf-8");
